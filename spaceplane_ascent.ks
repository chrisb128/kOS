...
local pilot is initAutopilot(spaceplane_ascent(), cfg).
pilot:initLogs().
...
until ship:altitude > 20000 and ship:velocity:surface:mag > 1200 and prevVel > ship:velocity:surface:mag {
...
until ship:altitude > 70000 {
    ...
    // burn to make and keep apoapsis above 80k
    if (ship:apoapsis < 80000 and throt = 1) {
        set throt to 1.
    } else if (ship:apoapsis < 80000) {
        set throt to 0.1.
    } else {
        set throt to 0.
    }
...
logInfo("Nav Mode    : circularize", 1).
